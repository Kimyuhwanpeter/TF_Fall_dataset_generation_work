...
def cal_loss(A_img, B_img, A2B_gener, B2A_gener, A_dis, B_dis):
    ...
    with tf.GradientTape() as g_tape, tf.GradientTape() as d_tape:
        ...
        g_loss = mae_criterion(DB_fake, tf.ones_like(DB_fake)) + mae_criterion(DA_fake, tf.ones_like(DA_fake)) \
                ...
                + (10.0 * abs_criterion(B_img, id_fake_A) + (10.0 * abs_criterion(A_img, id_fake_B)))
...
def main():
    # 2D 컨볼루션을 이용하되, 지금 각각 16 프레임씩 있다. A이미지에 대해 16 프레임, B 이미지 16프레임을 각각 가지고 온뒤
    # 모델에 입력으로 둘때 배치 하나당 16번의 이미지가 입력될 수 있도록 만들어보자
            ...
            for step in range(train_idx):
                ...
                for i in range(FLAGS.batch_size):
                    ...
                    for j in range(FLAGS.frames):
                        g_loss, d_loss = cal_loss(batch_A_img[:, j, :, :, :],
                                                  batch_B_img[:, j, :, :, :],
                                                  G_a2b_model,
                                                  G_b2a_model,
                                                  D_a_model,
                ...
                g_ /= FLAGS.frames
                d_ /= FLAGS.frames
                print("Epoch: {} [{}/{}] g_loss = {}, d_loss = {}".format(epoch, step + 1, train_idx, g_, d_))
