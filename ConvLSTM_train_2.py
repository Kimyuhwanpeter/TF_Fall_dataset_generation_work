...
flags.DEFINE_string("pre_checkpoint_path", "C:/Users/Yuhwan/Downloads/val", "Restore the checkpoint files")
...
flags.DEFINE_string("test_dir", "B2A", "")

flags.DEFINE_string("te_A_img_path", "D:/[1]DB/[4]etc_experiment/Fall_dataset/[5]TCL_dataset/GAN_data/divide/Standup_Fallstand/test/A_test", "")

flags.DEFINE_string("te_B_img_path", "D:/[1]DB/[4]etc_experiment/Fall_dataset/[5]TCL_dataset/GAN_data/divide/Standup_Fallstand/test/B_test", "")

flags.DEFINE_string("A_output", "D:/[1]DB/[4]etc_experiment/Fall_dataset/[5]TCL_dataset/GAN_data/divide/Standup_Fallstand/test/result/A_output", "")

flags.DEFINE_string("B_output", "D:/[1]DB/[4]etc_experiment/Fall_dataset/[5]TCL_dataset/GAN_data/divide/Standup_Fallstand/test/result/B_output", "")
...
def test_func(img, lab):
    ...
    #label = lab     # for binary
    label = tf.one_hot(int(lab), 14)    # for multi
...
def main(argv=None):
    ...
    if FLAGS.train:
        ...
        if FLAGS.test_dir == "A2B":
            print("============================================")
            print("Select the A2B direction to make fake images")
            print("============================================")
            A_folder_list = os.listdir(FLAGS.te_A_img_path)
            A_folder_list = [folder for folder in A_folder_list if folder !=".DS_Store"]
            A_folder_list = all_data(A_folder_list, FLAGS.te_A_img_path)
            print(len(A_folder_list))

            A_img, A_lab = zip(*A_folder_list)
            A_img, A_lab = np.array(A_img), np.array(A_lab)
            A_gener = tf.data.Dataset.from_tensor_slices((A_img, A_lab))
            A_gener = A_gener.map(test_func)
            A_gener = A_gener.batch(1)
            A_gener = A_gener.prefetch(tf.data.experimental.AUTOTUNE)
            ...
            for i in range(A_idx):
                ...
                fake_B = run_model(G_a2b_model, A_images, False)

                for j in range(FLAGS.frames):
                    fakeB = fake_B[0][j]
                    realA = A_images[0][j]
                    fakeB_folder = FLAGS.A_output + "/" + "fake_B_{}".format(i)
                    realA_folder = FLAGS.A_output + "/" + "real_A_{}".format(i)
                    ...
                    plt.imsave(fakeB_folder + "/fake_B_{}.jpg".format(j), fakeB * 0.5 + 0.5)
                    plt.imsave(realA_folder + "/real_A_{}.jpg".format(j), realA * 0.5 + 0.5)
                print("Made {} folder list..!!".format(i + 1))


        else:
            print("============================================")
            print("Select the B2A direction to make fake images")
            print("============================================")
            B_folder_list = os.listdir(FLAGS.te_B_img_path)
            B_folder_list = [folder for folder in B_folder_list if folder !=".DS_Store"]
            B_folder_list = all_data(B_folder_list, FLAGS.te_B_img_path)
            print(len(B_folder_list))
        
            B_img, B_lab = zip(*B_folder_list)
            B_img, B_lab = np.array(B_img), np.array(B_lab)
            B_gener = tf.data.Dataset.from_tensor_slices((B_img, B_lab))
            B_gener = B_gener.map(test_func)
            B_gener = B_gener.batch(1)
            B_gener = B_gener.prefetch(tf.data.experimental.AUTOTUNE)
            ...
            for i in range(B_idx):
                ...
                fake_A = run_model(G_b2a_model, B_images, False)
                for j in range(FLAGS.frames):
                    fakeA = fake_A[0][j]
                    realB = B_images[0][j]
                    fakeA_folder = FLAGS.B_output + "/" + "fake_A_{}".format(i)
                    realB_folder = FLAGS.B_output + "/" + "real_B_{}".format(i)
                    ...
                    plt.imsave(fakeA_folder + "/fake_A_{}.jpg".format(j), fakeA * 0.5 + 0.5)
